...
fname = '../sourcedata/unimelb_training.csv'
setwd('~/Downloads/DSR/Kaggle Grants/pythonGrants/')
raw <- read.csv(fname)
...
setkey(v2, Grant.Application.ID)
v2[,.N,by=Grant.Application.ID]
# select id_application, count(id_person) from "Grants".investigators group by id_application ORDER BY 1
...
v3 = v2[,.N,by=list(Grant.Application.ID, shortRole)]
v3[,shortRole:=paste("num",shortRole,sep="")]
dcast(v3, Grant.Application.ID ~ shortRole)

# select id_application, tx_role, count(id_person) from "Grants".investigators group by id_application, tx_role ORDER BY 1, 2
library(reshape2)
######################################################################
...
v3=v2[,.N,by=list(Grant.Application.ID, shortRole, Year.of.Birth)]
dcast(v3, Grant.Application.ID ~shortRole + Year.of.Birth)
...
v3=v2[,.N,by=list(Grant.Application.ID, shortRole, Country.of.Birth)]
dcast(v3, Grant.Application.ID ~ shortRole + Country.of.Birth)[1:10,,]
...
# [7] "DR.OtherLang"  
v3=v2[,.N,by=list(Grant.Application.ID, shortRole, Home.Language)]
dcast(v3, Grant.Application.ID, ~ shortRole + Home.Language)
...
# [7] "SR.PhD"
v3=v2[,.N,by=list(Grant.Application.ID, shortRole, With.PHD)]
dcast(v3, Grant.Application.ID ~ shortRole + With.PHD)[1:10,,]
...
summarized <- merge(investCountry, investDOB)
